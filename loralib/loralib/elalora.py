...
##############################################################################
    def update_and_mask(self, model, global_step):
        # Update the importance scores
        self.update_ipt(model)

        # Prune the lowest k triplets


        # Increase the rank of the model

        return

    def prune_lowest_k_triplets(self, model, k):
        prune_targets = []
        for n, p in model.named_parameters():
            if "lora_A" in n or "lora_B" in n or "lora_E" in n:
                # Calculate importance scores for the parameter triplets
                importance_scores = self.calculate_score(n, p, metric="ipt")
                sorted_indices = importance_scores.argsort()[:k]  # Get lowest k
                prune_targets.append((n, sorted_indices))
        # Prune based on collected targets
        self.apply_pruning(prune_targets)

    def expand_important_triplets(self, model, b):
        expand_targets = []
        for n, p in model.named_parameters():
            if "lora_A" in n or "lora_B" in n or "lora_E" in n:
                # Calculate importance scores for the parameter triplets
                importance_scores = self.calculate_score(n, p, metric="ipt")
                top_indices = importance_scores.argsort()[-b:]  # Get top b from least important
                expand_targets.append((n, top_indices))
        # Expand based on collected targets
        self.apply_expansion(expand_targets)

    def initialize_orthonormal(self, rank):
        # Use Gram-Schmidt or similar process to initialize `lora_A`, `lora_B` orthonormally
        self.lora_A.weight.data = self.gram_schmidt_initialization(self.lora_A.weight, rank)
        self.lora_B.weight.data = self.gram_schmidt_initialization(self.lora_B.weight, rank)
        # Initialize `lora_E` with low values
        self.lora_E.data.fill_(0.1)

    def apply_pruning(self, prune_targets):
        """
        Applies in-place pruning to the specified triplets in the LoRA matrices by removing arbitrary rows/columns.
        
        Args:
            prune_targets (list of tuples): Each tuple contains the parameter name and the indices of triplets to prune.
        """
        for param_name, indices in prune_targets:
            param = getattr(self.model, param_name)  # Access the parameter (e.g., self.model.lora_A)

            # Create a mask that keeps all rows except the ones in `indices`
            mask = torch.ones(param.size(0), dtype=torch.bool, device=param.device)
            mask[indices] = False  # Set the rows to be pruned as False

            # Apply the mask to keep only the rows/columns that are not pruned
            with torch.no_grad():
                pruned_weight = param[mask, :]  # Select rows based on the mask
                param.set_(pruned_weight)  # In-place modification of the weight matrix

            # Update optimizer state to remove pruned entries
            self.update_optimizer_state(param, mask)

    def update_optimizer_state(self, param, mask):
        """
        Updates the optimizer state after pruning specific rows/columns.
        
        Args:
            param (torch.nn.Parameter): The parameter that has been pruned.
            mask (torch.BoolTensor): A boolean mask indicating the rows to keep.
        """
        param_id = id(param)
        if param_id in self.optimizer.state:
            param_state = self.optimizer.state[param_id]

            # Prune `exp_avg` and `exp_avg_sq` or other optimizer-specific states
